...
| 신뢰성     | 같은 방의 일정만 표시되고 다른 방의 일정과 섞이지 않아야 한다.    | roomId 기준 조회 및 저장 적용                                                |
...
일정 기능을 실제 데이터 기반으로 구현하기 위해 `schedules` 테이블을 설계하고 SQL 파일로 기록하였다.
...
* `room_id` 인덱스 생성
* RLS 활성화
* 개발 및 과제 검증용 public CRUD policy 설정
* Supabase Realtime publication 등록
* DELETE 이벤트 처리를 위한 `replica identity full` 설정

현재 RLS 정책은 과제 및 개발 환경에서 익명 사용자의 CRUD를 허용하기 위해 `using (true)` 형태로 설정하였다. 이는 구현과 테스트에는 적합하지만, 실제 운영 환경에서는 방 참여자 검증 기반 정책으로 강화해야 한다.
